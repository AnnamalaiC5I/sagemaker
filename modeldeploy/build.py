...
def extend_config(args, model_package_arn, stage_config):
    ...
    # client.put_object(Body=json_object, Bucket='sagemaker-project-p-tp8lsnxnforl', Key='sagemaker-project-x6/extended-state-config.json')
    return {
        "Parameters": {**stage_config["Parameters"], **new_params},
        "Tags": {**stage_config.get("Tags", {}), **new_tags},
    }
...
if __name__ == "__main__":
    ...
    with open(args.import_staging_config, "r") as f:
        ...
        json_object = json.dumps(staging_config, indent = 4) 

        client = boto3.client('s3')
        client.put_object(Body=json_object, Bucket='sagemaker-project-p-tp8lsnxnforl', Key='sagemaker-project-x6/extended-stage-config.json')
    ...
    with open(args.import_prod_config, "r") as f:
        ...
        json_object = json.dumps(prod_config, indent = 4) 

        client = boto3.client('s3')
        client.put_object(Body=json_object, Bucket='sagemaker-project-p-tp8lsnxnforl', Key='sagemaker-project-x6/extended-prod-config.json')
